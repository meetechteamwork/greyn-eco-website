...
export default function CarbonRetirementPage() {
  ...
  const [activeTab, setActiveTab] = useState<'retired' | 'ledger'>('retired');

  useEffect(() => {
    // TODO: Replace with actual API call when carbon retirement API is available
    // For now, show empty state
    setLoading(false);
  }, []);

  const totalRetired = retiredCredits.reduce((sum, credit) => sum + credit.quantity, 0);
  const totalValue = retiredCredits.reduce((sum, credit) => sum + credit.totalAmount, 0);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xl font-semibold text-gray-700">Loading retirement data...</p>
        </div>
      </div>
    );
  }

  const mockRetiredCredits = retiredCredits;
  const mockLedgerEntries = ledgerEntries;

  // Mock data removed - using real data from API
  const mockLedgerEntriesData = [
    {
      id: '2',
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {retiredCredits.length > 0 ? retiredCredits.map((credit) => (
                  <tr
                    key={credit.id}
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center">
                      ...
                      <p className="text-sm text-gray-600">No retired credits found</p>
                    </td>
                  </tr>
                )}
              </tbody>
