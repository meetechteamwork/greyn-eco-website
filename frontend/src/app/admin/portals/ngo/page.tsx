...
export default function NGOPortalPage() {
  ...
  const [entities, setEntities] = useState<NGOEntity[]>([]);
  ...
  const fetchDashboard = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await api.admin.portals.ngo.getDashboard();
  ...
  const handleAction = async (action: 'disable' | 'review' | 'approve', entityId: string) => {
    ...
    try {
      const res = await api.admin.portals.ngo.updateEntityStatus(entityId, action);
      if (res.success) {
  ...
  const sessionPct =
    health?.maxSessions != null && health.maxSessions > 0
      ? Math.min(100, ((health.activeSessions ?? 0) / health.maxSessions) * 100)
      : 0;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-slate-400">Loading NGO Portal...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="divide-y divide-gray-200 dark:divide-slate-700">
                  {entities.length === 0 ? (
                    <div className="p-12 text-center text-gray-500 dark:text-slate-400">
                      <p className="font-medium">No NGOs in the database.</p>
                      <p className="text-sm mt-1">
                        To add sample data, run in the backend folder:{' '}
                        <code className="bg-gray-100 dark:bg-slate-700 px-2 py-1 rounded text-xs">npm run seed:ngo-portal</code>
                      </p>
                    </div>
                  ) : (
                    entities.map((entity) => (
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-gray-200 dark:border-slate-700 p-6">
                ...
                {health === null ? (
                  <div className="text-sm text-gray-500 dark:text-slate-400">
                    ...
                    <p className="mt-1">
                      Run <code className="text-xs bg-gray-100 dark:bg-slate-700 px-1 rounded">npm run seed:ngo-portal</code> in
                      the backend to create a health record.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-semibold text-gray-900 dark:text-white">
                        {health.uptime != null ? `${health.uptime}%` : '—'}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                    ...
                    <div className="pt-4 border-t border-gray-200 dark:border-slate-700">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-xs font-semibold text-gray-900 dark:text-white">
                          {(health.activeSessions ?? 0).toLocaleString()}
                        </span>
                      </div>
                      {health.maxSessions != null && health.maxSessions > 0 ? (
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-gray-200 dark:border-slate-700 p-6">
                ...
                <div className="space-y-4">
                  {activities.length === 0 ? (
                    <p className="text-sm text-gray-500 dark:text-slate-400">
                      No recent activity in the database. Seed with <code className="text-xs">npm run seed:ngo-portal</code> in
                      the backend folder.
                    </p>
                  ) : (
                ...
                <button
                  onClick={() => router.push('/admin/portals/ngo/activities')}
                  className="mt-4 w-full text-center text-sm font-semibold text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 transition-colors"
                >
                  View All Activity →
                </button>
              </div>
